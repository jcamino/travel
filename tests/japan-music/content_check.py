...
def block_fragments(page: str, minlen: int = 24):
    """Fragment each block separately so day-boundary chrome in the built
    page cannot break a source sentence that happened to sit across two
    adjacent blocks in the flat markdown HTML."""
    pieces = re.split(r"</(?:p|h[1-6]|li|td|th|div|tr)>", page)
    out = []
    for piece in pieces:
        out.extend(fragments(strip(piece), minlen))
    return out
...
def main(target: str) -> int:
    ...
    for frag in checked:
        ...
        if len(f) < 24:
            continue
        if re.sub(r"\s", "", f) not in dst_flat:
            missing.append(frag)
    ...
    print(f"fragments checked {len(checked)}, missing {len(missing)}")
