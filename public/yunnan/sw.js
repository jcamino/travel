...
var WAIT_NEW = 6000;    /* ms to wait for the newer page itself, once we know there is one */

function reload(u) { return new Request(u, {cache: 'reload'}); }

/* The page only; pictures come on 'warm'. Each file on its own, and a failure is swallowed: addAll
   rejects whole if any one request is not ok, and a proxy answering 403 for a single file would then
   fail the install, leaving the old worker in charge of every navigation with no way back. A build
   that installs with nothing cached still serves: the fetch handler falls through to the network and
   caches what it gets. */
self.addEventListener('install', function (e) {
  self.skipWaiting();
  e.waitUntil(caches.open(C).then(function (c) {
    return Promise.all(PAGE.map(function (u) { return c.add(reload(u)).catch(function () {}); }));
  }));
});
...
/* The page itself: the cached copy unless the server says there is a newer build. WAIT bounds the
   build.txt probe only; once there is a newer build to fetch, WAIT_NEW bounds that. Racing both
   against WAIT together, as this did, made WAIT_NEW unreachable. The newer page is fetched at './',
   never at the request's own URL: entering at /yunnan rather than /yunnan/ redirects, and the
   redirect check below would then throw on every load and pin the stale page for ever. */
function page(r, hit) {
  var probe = fetch('./build.txt', {cache: 'no-store'}).then(function (res) {
    if (!res.ok) throw new Error('no build.txt');
    return res.text();
  });
  return Promise.race([probe, timeout(WAIT)]).then(function (b) {
    if (b.trim() === C) return hit;
    var fresh = fetch(new Request('./', {cache: 'no-store'})).then(function (res) {
      if (!res.ok) throw new Error('bad page');
      var copy = res.clone();
...
self.addEventListener('fetch', function (e) {
  ...
  if (r.mode === 'navigate') {
    e.respondWith(caches.match('./').then(function (hit) {
      if (hit) return page(r, hit);
      /* nothing cached for this build: take the network and keep it, so offline works from now on */
